...
- First-class `NvidiaModel` adapter for NVIDIA API Catalog chat completions,
  using `NVIDIA_API_KEY`, the hosted NVIDIA endpoint, and provider-specific
  replay metadata.
...
- The changelog structure check moved from Python scripts into the Rust
  `vogon-xtask` tool to reduce long-lived non-Rust project tooling.
- The pull request template check moved from Python scripts into the Rust
  `vogon-xtask` tool while preserving README checklist coverage.
- The contributing checklist check moved from Python scripts into the Rust
  `vogon-xtask` tool while preserving live provider workflow guidance checks.
- The release checklist check moved from Python scripts into the Rust
...
- The package verification docs check moved from Python scripts into the Rust
  `vogon-xtask` tool while preserving offline package rationale checks.
- The container policy check moved from Python scripts into the Rust
...
- The issue template check moved from Python scripts into the Rust
  `vogon-xtask` tool while preserving open-source reporting guardrails.
- The deployment docs check moved from Python scripts into the Rust
...
- The Cargo metadata JSON artifact validator moved from Python scripts into the
  Rust `vogon-xtask` tool while preserving release package checks.
- The providers JSON validator moved from Python scripts into the Rust
...
- The replay verification JSON validator moved from Python scripts into the
...
- Release archive smoke tests now validate packaged binary and docs with a
  reusable script.
- Release artifact download smoke tests now extract downloaded CLI archives and
