...
Use the real version number in place of `v0.2.0`.
...
- Writes `cargo metadata --locked` dependency metadata as
  `vogon-v0.2.0-cargo-metadata.json`.
- Writes an SPDX 2.3 JSON SBOM as `vogon-v0.2.0-cargo-spdx.json`.
...
```sh
sha256sum -c vogon-v0.2.0-linux-x86_64.tar.gz.sha256
tar -xzf vogon-v0.2.0-linux-x86_64.tar.gz
./vogon --version
```
...
```powershell
$expected = (Get-Content .\vogon-v0.2.0-windows-x86_64.zip.sha256).Split()[0]
$actual = (Get-FileHash -Algorithm SHA256 .\vogon-v0.2.0-windows-x86_64.zip).Hash.ToLowerInvariant()
if ($actual -ne $expected) { throw "Checksum mismatch" }
Expand-Archive .\vogon-v0.2.0-windows-x86_64.zip -DestinationPath .\vogon-release -Force
.\vogon-release\vogon.exe --version
```

Use the real version number in place of `v0.2.0`.

The release also publishes `vogon-v0.2.0-cargo-metadata.json`,
`vogon-v0.2.0-cargo-spdx.json`, and matching `.sha256` files. Verify them the
same way before inspecting locked dependency metadata or SBOM contents:

```sh
sha256sum -c vogon-v0.2.0-cargo-metadata.json.sha256
sha256sum -c vogon-v0.2.0-cargo-spdx.json.sha256
```

The release also publishes a container image archive and checksum:

```sh
sha256sum -c vogon-v0.2.0-container-image.tar.gz.sha256
docker load --input vogon-v0.2.0-container-image.tar.gz
cargo run -p vogon-xtask -- check-container-image vogon-runtime:v0.2.0 --expected-version v0.2.0 --expected-revision <release-commit-sha>
docker run --rm vogon-runtime:v0.2.0 --version
docker run --rm --read-only vogon-runtime:v0.2.0 --version
docker run --rm --read-only vogon-runtime:v0.2.0 doctor --json | cargo run -p vogon-xtask -- check-doctor-json
mkdir -p target/container-smoke
chmod 777 target/container-smoke
docker run --rm --read-only -v "$PWD/target/container-smoke:/work" vogon-runtime:v0.2.0 init --force --output /work/starter.toml
docker run --rm --read-only -v "$PWD/target/container-smoke:/work:ro" vogon-runtime:v0.2.0 check --json /work/starter.toml | cargo run -p vogon-xtask -- check-workflow-json --expected-workflow-name starter-workflow --expected-step-count 2
```

Use the real version number in place of `v0.2.0` and the release commit SHA in
...
```sh
gh attestation verify vogon-v0.2.0-linux-x86_64.tar.gz -R kaleab-kali/vogon-runtime
gh attestation verify vogon-v0.2.0-container-image.tar.gz -R kaleab-kali/vogon-runtime
```

On Windows PowerShell:

```powershell
gh attestation verify .\vogon-v0.2.0-windows-x86_64.zip -R kaleab-kali/vogon-runtime
```
