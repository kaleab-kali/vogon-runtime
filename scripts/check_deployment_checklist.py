#!/usr/bin/env python3
"""Validate that deployment smoke commands stay in local and release checks."""
...
DEPLOYMENT_MARKER = "Before publishing or deploying an image, run:"
...
def check_repository(root: Path) -> list[str]:
    ...
    deployment_commands = extract_shell_commands(deployment_doc, DEPLOYMENT_MARKER)
    errors: list[str] = []
    ...
    if not deployment_commands:
        errors.append("docs/deployment.md: missing deployment smoke command block")
    ...
    for command in deployment_commands:
        if command not in readme_command_set:
            errors.append(f"README.md: missing deployment smoke command `{command}`")
        if command not in release_command_set:
            errors.append(f"docs/release.md: missing deployment smoke command `{command}`")
