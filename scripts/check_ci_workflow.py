...
REQUIRED_SNIPPETS = {
    ...
    "clippy check": "cargo clippy --workspace --all-targets --all-features -- -D warnings",
    "workspace tests": "cargo test --workspace --all-features",
    "deterministic-only CLI build": "cargo check -p vogon-cli --no-default-features",
    "MSRV test": "cargo +1.85.0 test --workspace --all-features --locked",
    "benchmark smoke": "cargo bench -p vogon-core --bench runtime -- --iterations 100",
    "release build": "cargo build --release --workspace --all-features",
    ...
    "core package verification": "cargo package -p vogon-core --allow-dirty --offline",
    "workspace package smoke": "cargo package --workspace --allow-dirty --no-verify --offline",
